...
\end{theorem}
\begin{proof}
	Since $\lambda$ is a homomorphism, we can prove it is injective by showing its kernal is trivial. Then
	\begin{equation}
...
\begin{theorem}(A $p$-group has a non-trivial center)
	Let $G$ be a $p$-group. Then $Z(G) = \{1\}$.  In words, there has to be a non-trivial element of the group that commutes with everything else.
\end{theorem}
